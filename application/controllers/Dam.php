...
class Dam extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['alert'] = $this->db->get('bug_report', ['status' => 0])->row_array();
        ...
        $this->load->view('templates/adm_head', $data);
        $this->load->view('pages/dam', $data);
    ...
    public function orderlist()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['orderan'] = $this->db->get_where('tbl_orderan', ['status' => 1])->result();
            $data['danger'] = $this->db->get_where('bug_report', ['status' => 0])->result();
            $data['alert'] = $this->db->get_where('bug_report', ['status' => 0])->row_array();
            $data['order'] = $this->db->get_where('tbl_orderan', ['status' => 1])->result();
            $this->load->view('templates/adm_head', $data);
    ...
    public function profile()
    {
        ...
        $data['name'] = $this->db->get_where('tbl_user_qr', ['phone' => $sesi])->row_array();
        $data['danger'] = $this->db->count_all_results('bug_report');
        $data['alert'] = $this->db->get_where('bug_report', ['status' => 0])->row_array();
    ...
    public function report_page()
    {
        ...
        //var_dump($this->session->userdata('level_id'));
        $data['title'] = "Reports Page - DUITIN App";
        ...
        $data['name'] = $this->db->get_where('tbl_user_qr', ['phone' => $sesi])->row_array();
        $data['report'] = $this->db->get('bug_report')->result();
        $data['danger'] = $this->db->get_where('bug_report', ['status' => 0])->result();
        $data['alert'] = $this->db->get_where('bug_report', ['status' => 0])->row_array();

        $this->load->view('templates/adm_head', $data);
        $this->load->view('pages/report_page', $data);
        $this->load->view('templates/footbar');
    }
    ...
    {
        $sesi = $this->session->userdata('phone');
        $data['title'] = "Report Detail Message";
        $data['name'] = $this->db->get_where('tbl_user_qr', ['phone' => $sesi])->row_array();
        $data['danger'] = $this->db->get_where('bug_report', ['status' => 0])->result();
        $data['alert'] = $this->db->get_where('bug_report', ['status' => 0])->row_array();

        //proses pengambilan data berdasarkan id_user
    ...
    public function tbl_developer()
    {
        $sesi = $this->session->userdata('phone');

        //var_dump($this->session->userdata('level_id'));
        $data['title'] = "DBase Developers - DUITIN App";

        $data['name'] = $this->db->get_where('tbl_user_qr', ['phone' => $sesi])->row_array();
        $data['danger'] = $this->db->get_where('bug_report', ['status' => 0])->result();
        $data['alert'] = $this->db->get_where('bug_report', ['status' => 0])->row_array();

        $data['admin'] = $this->db->get_where('tbl_user_qr', ['level_id' => 1])->result();
        $data['ceo'] = $this->db->get_where('tbl_user_qr', ['level_id' => 2])->result();
        $data['accounting'] = $this->db->get_where('tbl_user_qr', ['level_id' => 3])->result();

        $data['developer'] = $this->db->get_where('tbl_user_qr', ['level_id <=' => 3])->result();

        $this->load->view('templates/adm_head', $data);
        $this->load->view('pages/tbl_developer', $data);
        $this->load->view('templates/footbar');
    }
}
